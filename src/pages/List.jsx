...
const List = () => {
  ...
  const currentList = lists[currentListIndex] || { title: "Untitled List", dueDate: "", items: [], remainingItemsCount: 0, totalItemsCount: 0, store: "" };
  ...
  const handleDeleteList = () => {
    ...
    if (confirmation) {
      const updatedLists = lists.filter((list, index) => index !== currentListIndex);
      setLists(updatedLists);
      updateLocalStorage(updatedLists);
      ...
      <div>
        <div>
          {lists.map((list, index) => (
            <Button key={index} onClick={() => handleListChange(index)}>{list.title}</Button>
          ))}
          <Button onClick={handleAddList}><AddIcon /> Add New List</Button>
          <Button onClick={handleDeleteList}>Delete Current List</Button>
        </div>
        <h2 className="text-xl font-medium text-gray-500">Today is {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric" })}</h2>
        <Input type="text" value={currentList.title} onChange={e => handleTitleChange(e.target.value)} disableUnderline={true} style={{ fontSize: "30px", fontWeight: 600 }} />
        <div className="flex items-center gap-[2px]">
          <span>Due date:</span>
          <Input type="date" value={currentList.dueDate} onChange={e => handleDueDateChange(e.target.value)} disableUnderline />
        </div>
        <div className="flex items-center">
          <span>Buy at:</span>
          <select className="ml-1 bg-transparent" onChange={e => handleStoreChange(e.target.value)} value={currentList.store}>
            {stores.map(option => (
        ...
        <p>Items remaining: {currentList.remainingItemsCount}/{currentList.totalItemsCount}</p>
        <CSList
          list={currentList}
          updateList={(updatedList) => {
            const updatedLists = lists.map((list, index) => index === currentListIndex ? updatedList : list);
            setLists(updatedLists);
            updateLocalStorage(updatedLists);
          }}
        />
